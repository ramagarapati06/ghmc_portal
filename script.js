...
function adminComplaintsPage(){
  ...
  return `<section class="container">${titleBlock("ADMIN / OFFICER • ALL COMPLAINTS","Complaint Management","Review every complaint in the system, verify reports, assign field staff and update resolution status.")}
  <div class="role-lock">ADMIN VIEW • This is the complete city-wide complaint register. Citizens and Field Staff do not have access to this page.</div>
  <div class="admin-toolbar"><input id="adminSearch" placeholder="Search ID, title, category or location"><select id="adminStatus"><option>All statuses</option><option>Submitted</option><option>Under Review</option><option>Assigned</option><option>In Progress</option><option>Resolved</option></select><select id="adminPriority"><option>All priorities</option><option>Low</option><option>Medium</option><option>High</option><option>Critical</option></select></div>
  <div class="admin-card" id="adminTable"></div></section>`;
}
...
function fieldWorkPage(){
  ...
  const staffName=userName(), assigned=complaints.filter(c=>c.assigned===staffName);
  return `<section class="container">${titleBlock("FIELD STAFF • MY ASSIGNED WORK","My Assigned Complaints","Only complaints assigned to ${esc(staffName)} are shown here. Update the work status after your field visit.")}
  <div class="role-lock">FIELD STAFF VIEW • You can update work status and remarks, but you cannot verify complaints or assign work.</div>
  <div class="field-list">${assigned.length?assigned.map(c=>`<div class="field-card"><div class="field-top"><div><span class="eyebrow">${esc(c.id)}</span><h3>${esc(c.title)}</h3></div>${badge(c.status)}</div>
  <div class="details"><div><small>Category</small><b>${esc(c.category)}</b></div><div><small>Location</small><b>${esc(c.location)}</b></div><div><small>Priority</small><b>${esc(c.priority)}</b></div><div><small>Submitted</small><b>${esc(c.submitted)}</b></div></div>
  <p>${esc(c.description)}</p>${c.image?`<img class="modal-image" src="${c.image}" alt="Complaint evidence">`:""}
  <label class="field"><span>Field work remark</span><input data-remark="${c.id}" value="${esc(c.remarks||"")}" placeholder="Describe the action taken"></label>
  <div class="field-actions"><button class="btn btn-secondary" data-work="${c.id}" data-value="In Progress">Mark In Progress</button><button class="btn btn-primary" data-work="${c.id}" data-value="Resolved">✓ Mark Resolved</button></div></div>`).join(""):`<div class="empty"><h3>No work assigned</h3><p>There are currently no complaints assigned to ${esc(staffName)}.</p></div>`}</div></section>`;
}
...
function setRoleNavigation(){
  ...
  if(r==="citizen"){
    items=[
      ["citizen","Dashboard"],["report","Report Issue"],["complaints","My Complaints"],
      ["track","Track Complaint"],["feedback","Feedback"],["contact","Contact"]
    ];
  }else if(r==="admin"){
    items=[
      ["admin","Command Centre"],["adminComplaints","All Complaints"],
      ["contact","Civic Contact"]
    ];
  }else if(r==="field"){
    items=[
      ["field","Dashboard"],["fieldWork","My Assigned Work"],
      ["track","Track Complaint"],["contact","Civic Contact"]
    ];
  }else{
    items=[
      ["home","Home"],["track","Track Complaint"],["contact","Contact"]
    ];
  }
  nav.innerHTML=items.map(([page,label])=>`<a href="#" data-page="${page}" class="nav-link ${currentPage===page?"active":""}">${label}</a>`).join("");
  bindPageLinks();
}
...
function render(){
  ...
  if(currentPage==="adminComplaints"&&r!=="admin")currentPage=r==="citizen"?"citizen":r==="field"?"field":"login";
  if(currentPage==="fieldWork"&&r!=="field")currentPage=r==="citizen"?"citizen":r==="admin"?"admin":"login";
  if(["report","complaints","feedback"].includes(currentPage)&&r!=="citizen")currentPage=r?"login":"login";

  const pages={home,login:loginPage,citizen:citizenDashboard,admin:adminPage,adminComplaints:adminComplaintsPage,field:fieldPage,fieldWork:fieldWorkPage,report,complaints:complaintsPage,track,feedback:feedbackPage,contact:contactPage};
  ...
  if(currentPage==="admin"||currentPage==="adminComplaints"){fillAdmin();["adminSearch","adminStatus","adminPriority"].forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener(id==="adminSearch"?"input":"change",fillAdmin);});}
  if(currentPage==="field"||currentPage==="fieldWork")setupField();
